...
namespace Ark
{
    //自身[烧伤]层数不低于15且剩余光芒足够则自动使用
    public class DiceCardSelfAbility_TLL12 : DiceCardSelfAbilityBase
    {
        public static string Desc = "自身[烧伤]层数不低于15且剩余光芒足够则自动使用";
        public override void OnEnterCardPhase(BattleUnitModel unit, BattleDiceCardModel self)
        {
            if (unit.allyCardDetail.GetHand().Contains(self) && unit.bufListDetail.GetKewordBufStack(KeywordBuf.Burn) >= 15 && unit.cardSlotDetail.PlayPoint >= self.GetCost())
            {
